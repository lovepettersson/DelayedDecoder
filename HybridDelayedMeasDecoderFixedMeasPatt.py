...
class LT_FullHybridDecoderOld(object):
    r"""
    """
    ...
    def decode_specific_output_qbt(self, input_strats, output_qbt, lost_output_qbt=[]):
        ...
        for key in succes_measured_stabs.keys():
            ...
            if len(s) > 1:
                ...
                # print(succes_measured_stabs[key][-1])
                # print(trans, lost, len(new_trans_list))
                # print()
            else:
                # pauli = succes_measured_stabs[key][0][0][3]
                # measured = succes_measured_stabs[key][1]
...
if __name__ == '__main__':
    ...
    patterns = [[1, 2, 3, 4, 6, 9, 7, 10, 5, 8]]
    graph_edges = [[0, 1], [0, 2], [0, 3], [0, 5], [0, 8], [0, 6], [0, 4], [0, 10], [1, 2], [1, 8], [1, 4], [1, 7], [2, 8], [2, 9], [3, 5], [3, 8], [3, 6], [3, 7], [5, 8], [5, 9], [5, 10], [6, 9], [6, 4], [6, 7], [9, 10], [4, 7], [7, 10]]
    ...
    for m_idx, meas_pattern in enumerate(patterns[:1]):
        ...
        # adaptive_decoder_old = LT_FullHybridDecoderOld(copy.deepcopy(gstate), copy.deepcopy(input_strats),
        ...
        # ld_dec_matt_qbts = adaptive_decoder.max_number_of_m_dec
    ...
    print(adaptive_decoder_new.analytic_exp)
    # log_trans_no_matt = adaptive_decoder_new_no_extra_matt.calculate_log_transmisison(t_list, adaptive_decoder_new_no_extra_matt.analytic_exp)
    ...
    # log_loss_hybrid_no_matt = [1 - t for t in log_trans_no_matt]
    plt.plot(loss_list, log_loss_hybrid, color="red", label="Code")
    # plt.plot(loss_list, log_loss_hybrid_no_matt, color="black", label="No delayed meas.")
